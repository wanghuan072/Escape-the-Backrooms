...
function GameInfoCard({ locale, card, labeled, entityCount, levelStats }: { locale: Locale; card: 'card01' | 'card02' | 'card03'; labeled?: boolean; entityCount: number; levelStats: ReturnType<typeof getLevelDirectoryStats> }) {
  return <div className="info-card"><h3>{translate(locale, `homePage.gameInfo.${card}Title`)}</h3><ul className="info-list">{Array.from({ length: 6 }, (_, index) => { const number = String(index + 1).padStart(2, '0'); const value = card === 'card02' && number === '01' ? `${levelStats.total} ${translate(locale, 'homePage.gameInfo.card02Item01Value')}` : card === 'card02' && number === '02' ? `${levelStats.main} ${translate(locale, 'homePage.gameInfo.card02Item02MainValue')} · ${levelStats.hidden} ${translate(locale, 'homePage.gameInfo.card02Item02HiddenValue')}` : card === 'card02' && number === '03' ? `${entityCount} ${translate(locale, 'homePage.gameInfo.card02Item03Value')}` : translate(locale, `homePage.gameInfo.${card}Item${number}${labeled ? 'Value' : ''}`); return <li key={number}>{labeled && <strong>{translate(locale, `homePage.gameInfo.${card}Item${number}Label`)}</strong>} {value}</li> })}</ul></div>
}
...
export default function HomePage({ locale }: { locale: Locale }) {
  ...
  const levelStats = getLevelDirectoryStats(locale)
    ...
    <div className="home-view">
      <section className="hero"><div className="container"><div className="hero-layout">
        <div className="hero-content">
          ...
          <div className="hero-stats">{[1, 2, 3].map((number) => <div className="stat-box" key={number}><div className="stat-number">{number === 1 ? levelStats.total : number === 2 ? levelStats.main : entities.length}</div><div className="stat-text">{translate(locale, `homePage.hero.stat0${number}Text`)}</div></div>)}</div>
      ...
      <section className="game-info"><div className="container"><SectionHeader locale={locale} section="gameInfo" /><div className="info-grid"><GameInfoCard locale={locale} card="card01" labeled entityCount={entities.length} levelStats={levelStats} /><GameInfoCard locale={locale} card="card02" labeled entityCount={entities.length} levelStats={levelStats} /><GameInfoCard locale={locale} card="card03" entityCount={entities.length} levelStats={levelStats} /></div></div><AdPlacement /></section>
