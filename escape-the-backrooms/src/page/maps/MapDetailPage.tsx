    ...
    <div className="map-detail-view">
      ...
      <section className="detail-content"><div className="container"><div className="content-layout">
        <main className="main-content">
          ...
          {mapSections?.length ? mapSections.map((section) => <section className="map-section" key={section.imageUrl}><h2>{section.title}</h2><div className="map-container"><div className="map-wrapper"><img src={section.imageUrl} alt={section.imageAlt} width={section.width} height={section.height} className="map-image" loading="lazy" />{section.callouts?.map((callout) => <span key={callout.id} className="map-callout" style={{ left: `${callout.x}%`, top: `${callout.y}%` }}>{callout.id}</span>)}</div></div><div className="map-points-container"><div className="map-points-grid">{section.mapPoints.map((point, index) => <div key={point.id ?? `${point.title}-${index}`} className="map-point-card"><div className="map-point-header"><h3 className="map-point-title">{point.title}</h3></div><p className="map-point-content">{point.content}</p></div>)}</div></div><div className="map-section-body v-html-style" dangerouslySetInnerHTML={{ __html: optimizeRichHtml(section.detailsHtml) }} /></section>) : <><div className="map-container"><div className="map-wrapper">{map.mapImageUrl ? <img src={map.mapImageUrl} alt={map.imageAlt || map.title} width={map.imageWidth} height={map.imageHeight} className="map-image" loading="lazy" /> : <div className="map-image placeholder">{map.title}</div>}</div></div>{map.galleryImages && map.galleryImages.length > 0 && <div className="map-gallery">{map.galleryImages.map((image) => <figure key={image.src} className="map-gallery-item"><figcaption>{image.title}</figcaption><img src={image.src} alt={image.alt} className="map-image" loading="lazy" /></figure>)}</div>}</>}
          <AdPlacement />
        ...
        <aside className="sidebar">
          <div className="image-card"><div className="image-header">{map.title}</div><div className="image-wrapper">{map.imageUrl ? <img src={map.imageUrl} alt={map.imageAlt || map.title} width={map.imageWidth} height={map.imageHeight} className="level-image" loading="lazy" /> : <div className="level-image placeholder">{map.title}</div>}</div></div>
          <AdPlacement />
