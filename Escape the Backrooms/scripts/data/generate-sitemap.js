...
/**
 * Preserve lastmod only while the content fingerprint is unchanged.
 * Existing URLs with changed content and brand-new URLs use today's date.
 */
function resolveLastmod(entry, cache, sitemapLastmods) {
  ...
  if (cached) {
    return { lastmod: today(), status: 'changed' }
  }
  if (sitemapLastmods[entry.loc]) {
    return { lastmod: sitemapLastmods[entry.loc], status: 'preserved' }
  }
  return { lastmod: today(), status: 'new' }
}
...
async function loadData(locale = 'en') {
  ...
  try {
    ...
    data.messages = messagesModule.default || {}
  } catch (error) {
    console.warn(`Failed to load messages data (${locale}):`, error.message)
  }

  try {
    const levelsModule = await import(`../../src/content/levels/${locale}.js`).catch(() =>
      import('../../src/content/levels/en.js'),
    )
  ...
  try {
    const mapsModule = await import(`../../src/content/maps/${locale}.js`).catch(() =>
      import('../../src/content/maps/en.js'),
    )
  ...
  try {
    const relatedGamesModule = await import('../../src/content/related-games.js')
  ...
  supportedLocales.forEach((locale) => {
    ...
    relatedGames.forEach((game) => {
      ...
      entries.push({
        ...
        priority: 0.7,
        changefreq: 'monthly',
        hash: fingerprint(JSON.stringify({ type: 'related-game', locale, game })),
      })
    })
  })
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  for (const entry of entries) {
    const { lastmod, status } = resolveLastmod(entry, cache, sitemapLastmods)
...
async function main() {
  try {
    ...
    const publicDir = path.dirname(PUBLIC_SITEMAP)
    ...
    console.log(`   Changed URLs (lastmod = today): ${stats.changedCount}`)
